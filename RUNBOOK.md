...
| Role   | Username | Name                                |
|--------|----------|--------------------------------------|
| Farmer | farmer1  | Ganesh Pawar                         |
| Farmer | farmer2  | Suresh Chavan                        |
| FPO    | fpo1     | Nashik Onion Producer Co. Ltd        |
| Buyer  | buyer1   | Maharashtra Agro Foods Pvt Ltd       |
| Buyer  | buyer2   | FreshChain Retail                    |
| Admin  | admin1   | MSIS Market Authority Desk           |

## 3a. Judge demo sequence (shortest reliable path, ~5 min)

1. Login as `farmer1` → Dashboard → **Market Intelligence**: pick a crop, see
...
7. Logout, login as `admin1` → **Admin Dashboard** (platform metrics) →
   **Grievances** → resolve the one just raised.
8. Logout, login as `farmer1` again → bell icon shows the new "grievance
   resolved" notification.
9. Logout, login as `buyer1` → **Marketplace** → submit an offer on an open
   lot → **Transactions** shows it.
10. Logout, login as `fpo1` → **FPO Aggregation** → shows member farmers'
